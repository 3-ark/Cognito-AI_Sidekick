...
import { extractMainContent } from './utils/scrapers';
...
export const processQueryWithAI = async (
  ...
  try {
   if (!apiUrl) {
    console.error('processQueryWithAI: apiUrl is not provided.');
...
\n
0. Using previous chat history as context to fully understand the user's actual intent.
...
export const webSearch = async (
    ...
    const dynamicReferer = `https://www.google.com/search?q=${encodeURIComponent(query)}`;
 ...
 ...result, content: '[Invalid URL]', status: 'error', 
};

                if (effectiveSignal.aborted) return {
 ...result, content: `[Fetching aborted by user: ${result.url}]`, status: 'aborted', 
};
                ...
                return {
 ...result, content: pageContent, status: pageStatus, 
};
            });
            ...
            searchResults.forEach((result, index) => {
                 ...
                 if (index < linksToFetch.length) {
                     ...
                     if (correspondingFetch?.status === 'fulfilled') {
                         ...
                         combinedResultsText += `Content: [Not fetched or status unknown]\n\n`;
                     }
            ...
            try {
                ...
                if (apiResponse && apiResponse.length > 0 && apiResponse[0].results) {
                    ...
                    apiResponse[0].results.forEach((block, index) => {
                      console.log(`[webSearch - Wikipedia] Processing result block ${index + 1}:`, {
title: block.document_title, section: block.section_title, summary_length: block.summary?.length, content_length: block.content?.length,
});
 ...
 ...result, content: '[Invalid URL]', status: 'error', 
                ...
                if (effectiveSignal.aborted) return {
 ...result, scrapedContent: `[Scraping aborted by user: ${result.url}]`, status: 'aborted', 
};
                ...
                return {
 ...result, scrapedContent: scrapedPageContent, status: pageStatus, 
};
            });
