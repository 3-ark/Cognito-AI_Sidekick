...
export const AvatarUpload = ({ personaName }: { personaName: string }) => {
  ...
  return (
    <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
          ...
          <TooltipTrigger asChild>
            <PopoverTrigger asChild>
              <Button
                variant="ghost"
                size="sm"
                className="p-0 rounded-full w-8 h-8 overflow-hidden border border-(--text)/20"
              >
                <img
                  src={currentAvatar}
                  alt={personaName}
      ...
      <PopoverContent
        className="w-64 p-4 bg-(--bg) border border-(--text)/20"
        align="start"
      >
        <div className="flex flex-col gap-3">
          <div className="text-sm font-medium text-(--text)">Change Avatar</div>
          <input
            type="file"
            ref={fileInputRef}
